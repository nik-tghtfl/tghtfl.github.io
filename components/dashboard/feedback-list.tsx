        ...
        >
          {item.actionRecommendation && (
            <div className="absolute top-3 right-3">
              <button
                type="button"
                className="text-amber-500 hover:text-amber-600 transition-colors focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-2 rounded touch-manipulation"
                aria-label="AI recommendation"
                onClick={() =>
                  setActiveTooltipId((prev) => (prev === item.id ? null : item.id))
                }
                onBlur={() =>
                  setActiveTooltipId((prev) => (prev === item.id ? null : prev))
                }
              >
                <Lightbulb className="h-4 w-4" />
              </button>
              {activeTooltipId === item.id && (
                <div className="absolute right-0 top-8 z-50 w-64 rounded-lg border border-gray-200 bg-white p-3 shadow-lg">
